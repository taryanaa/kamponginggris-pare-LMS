    ...
    <div id="app">
        ...
        <div id="main">
            ...
            <div class="page-heading">
                <h3>Welcome, Mentor {{ auth()->user()->name }}!</h3>
            </div>
                    ...
                    <div class="col-12 col-lg-8">
                                    ...
                                    <div class="card-body px-4 py-4-5">
                                        <div class="row">
                                            ...
                                            <div class="col-md-8 col-lg-12 col-xl-12 col-xxl-7">
                                                ...
                                                <h6 class="font-extrabold mb-0">{{ $totalMaterials ?? 0 }}</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card">
                                    <div class="card-body px-4 py-4-5">
                                        <div class="row">
                                            ...
                                            <div class="col-md-8 col-lg-12 col-xl-12 col-xxl-7">
                                                <h6 class="text-muted font-semibold">Upcoming Sessions</h6>
                                                <h6 class="font-extrabold mb-0">{{ $upcomingSessions ?? 0 }}</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card">
                                    <div class="card-body px-4 py-4-5">
                                        <div class="row">
                                            ...
                                            <div class="col-md-8 col-lg-12 col-xl-12 col-xxl-7">
                                                <h6 class="text-muted font-semibold">Pending Reviews</h6>
                                                <h6 class="font-extrabold mb-0">{{ $pendingReviews ?? 0 }}</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4>Student Performance (Average Score)</h4>
                            </div>
                            <div class="card-body">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-hover table-lg">
                                        ...
                                        <tbody>
                                            @if(isset($upcomingSessionsDetail) && $upcomingSessionsDetail->count() > 0)
                                                @foreach($upcomingSessionsDetail as $session)
                                                    <tr>
                                                        <td class="text-bold-500">{{ $session->student->name ?? 'Student' }}</td>
                                                        <td>{{ \Carbon\Carbon::parse($session->schedule_date)->format('M d, Y') }}</td>
                                                        <td class="text-bold-500">{{ $session->schedule_time }}</td>
                                                        <td><span class="badge bg-primary">{{ $session->lesson_type }}</span></td>
                                                    </tr>
                                                @endforeach
                                            @else
                                                <tr>
                                                    <td colspan="4" class="text-center text-muted">No upcoming sessions</td>
                                                </tr>
                                            @endif
                                        </tbody>
                                    </table>
    ...
    <div class="card-body py-4 px-4">
        <div class="d-flex align-items-center">
            ...
            <div class="ms-3 name">
                ...
                <h6 class="text-muted mb-0">Mentor</h6>
            </div>
        </div>
    </div>
</div>
...
<div class="card">
    ...
    <div class="card-content pb-4">
        @if(isset($recentActivities) && $recentActivities->count() > 0)
            ...
            @php
                $studentPhoto = $activity->student && $activity->student->profile_photo 
                    ? '/dashboard/storage/app/public/profile_photos/' . $activity->student->profile_photo 
                    : '/dashboard/assets/compiled/jpg/' . (($loop->index % 4) + 1) . '.jpg';
            @endphp
            <img src="{{ $studentPhoto }}" 
                 alt="{{ $activity->student->name ?? 'Student' }}"
                 onerror="this.onerror=null; this.src='/dashboard/assets/compiled/jpg/{{ (($loop->index % 4) + 1) }}.jpg';">
                                </div>
                                <div class="name ms-4">
                                    <h5 class="mb-1">{{ $activity->student->name ?? 'Student' }}</h5>
                                    <h6 class="text-muted mb-0">Submitted '{{ $activity->task->title ?? 'Task' }}'</h6>
                                </div>
                            </div>
                        @endforeach
                    @else
                    ...
                    @endif
                    <div class="px-4">
                        <a href="{{ route('mentor.tasks') }}" class='btn btn-block btn-xl btn-outline-primary font-bold mt-3'>View All Activities</a>
                    </div>
                </div>
            </div>
                    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            var optionsStudentProgress = {
                series: [{{ $studentPerformance->avg_score ?? 75 }}],
                chart: {
                    height: 350,
                    type: 'radialBar',
                    toolbar: {
